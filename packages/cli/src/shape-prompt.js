...
자세히 검토받을 수 있어요. 응답은 자유 형식이라 사용자가 읽고 shape-architecture.yml을 수동으로
...
export function buildArchitectureReviewPromptMarkdown({
  ...
  const body = [
    ...
    '이 섹션은 사용자가 받은 응답을 다시 byeorim에 붙여넣을 때 코드가 자동으로 읽어 shape-architecture.yml에 반영하는 자리예요.',
    '자유 형식으로만 답하셔도 괜찮아요. 그때는 사용자가 응답을 직접 읽고 shape-architecture.yml을 손으로 다듬어요.',
    '',
    '형식.',
    '',
    '````markdown',
    ...
    '## 도메인 적합성',
    '',
    '7항목 답변에 "처음 만드는 결"이 자주 등장해 java + 한 덩어리 코드 구조는 무겁게 박힐 수 있어요. 다만 "안정적 운영"을 강조하셔서 java 결이 어울리는 자리도 함께 봐요.',
    '',
    '## 결정 사이 트레이드오프',
    '',
    'h2 인메모리 + java + 한 덩어리는 빠른 시작에 맞지만 운영 중 재시작하면 데이터가 사라지는 결이 있어요. sqlite로 옮기면 한 파일로 가벼우면서 보존됩니다.',
    '',
    '## 시작 무게',
    '',
    '4개 결정 중 데이터베이스 한 자리만 옮기는 결로 충분해 보여요.',
    '',
    '## 제안된 변경 사항',
    '',
    '```yaml',
    ...
    '    target_key: database',
    '    new_value: sqlite',
    '    reason: h2 인메모리는 재시작 시 데이터가 사라져요. sqlite는 한 파일로 가벼우면서 보존됩니다',
    '```',
    '````',
  ].join('\n');
  return HEADER + body + '\n' + FOOTER;
