...
// 기본 spawnProcess. child_process.spawn 래퍼로 stdout/stderr를 줄 단위로 onStdout/onStderr에 전달.
// Windows 호환을 위해 shell: true. exited Promise는 close 이벤트에 resolve.
function defaultSpawnProcess({ cmd, args, cwd, onStdout, onStderr }) {
...
// 기본 fetchHealth. fetch + AbortController로 timeout 적용. 에러나 non-200은 false.
async function defaultFetchHealth(url, { timeoutMs = 1500 } = {}) {
  try {
...
async function pollUntilReady(url, { fetchFn, timeoutMs, intervalMs, signal }) {
