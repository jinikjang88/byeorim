// 어댑터 선택. ADR 0024 결정 3의 환경 변수 BYEORIM_AI_ADAPTER 표가 한 자리에 모인다.
...
// (ADR 0009 결정 4의 의존성 주입). 이 함수는 bin/byeorim.js 같은 엔트리에서만 호출된다.
...
// 실제 인증은 브릿지가 처리한다.
const BRIDGE_PLACEHOLDER_KEY = 'byeorim-bridge-placeholder';
  ...
  if (!apiKey && !baseURL) {
    throw new Error(
      'Claude 어댑터를 쓰려면 ANTHROPIC_API_KEY를 설정해주세요. ' +
        ...
        '비용 없이 흐름만 확인하려면 BYEORIM_AI_ADAPTER=mock으로 두세요',
    );
  }
