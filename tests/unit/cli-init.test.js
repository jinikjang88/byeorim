...
function makeTempCwd() {
  return mkdtempSync(join(tmpdir(), 'byeorim-init-'));
}

test('빈 디렉토리에서 .byeorim/ 작업 공간을 만든다', () => {
  // Given: 비어있는 임시 디렉토리
  ...
  try {
    ...
    assert.equal(existsSync(result.byeorimDir), true);
    assert.equal(existsSync(join(result.byeorimDir, 'project')), true);
...
test('diary.md는 사용자에게 다이어리 의도를 안내하는 한국어 헤더로 시작한다', () => {
  ...
  try {
    ...
    const content = readFileSync(join(cwd, '.byeorim', 'project', 'diary.md'), 'utf8');
...
test('.gitignore는 project/generated/만 무시한다', () => {
  ...
  try {
    runInit({ cwd });
    // When: .byeorim/.gitignore를 읽는다
    const content = readFileSync(join(cwd, '.byeorim', '.gitignore'), 'utf8');
...
test('이미 .byeorim/이 있으면 한국어 메시지로 거부한다', () => {
  // Given: 한 번 init한 자리
  const cwd = makeTempCwd();
  try {
    runInit({ cwd });
    ...
    assert.throws(
      () => runInit({ cwd }),
      (err) => {
        assert.match(err.message, /\.byeorim\/ 작업 공간이 이미 있습니다/);
