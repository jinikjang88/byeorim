...
test('BYEORIM_AI_ADAPTER=mock는 mock 어댑터를 만든다', () => {
  withEnv({ BYEORIM_AI_ADAPTER: 'mock' }, () => {
    const adapter = selectAdapter();
    assert.equal(adapter.name, 'mock');
  });
});
...
test('인자로 들어온 name이 환경 변수보다 우선시된다', () => {
  withEnv({ BYEORIM_AI_ADAPTER: 'claude' }, () => {
    const adapter = selectAdapter({ name: 'mock' });
    assert.equal(adapter.name, 'mock');
  });
});

test('BYEORIM_AI_ADAPTER=claude는 ANTHROPIC_API_KEY가 있으면 claude 어댑터를 만든다', () => {
  withEnv({ BYEORIM_AI_ADAPTER: 'claude', ANTHROPIC_API_KEY: 'sk-test' }, () => {
    const adapter = selectAdapter();
...
test('ANTHROPIC_BASE_URL만 있으면 키 없이도 claude 어댑터를 만든다', () => {
  ...
  withEnv({ BYEORIM_AI_ADAPTER: 'claude', ANTHROPIC_BASE_URL: 'http://localhost:3000' }, () => {
    const adapter = selectAdapter();
    assert.equal(adapter.name, 'claude');
  });
});
...
test('알 수 없는 어댑터 이름은 지원 목록과 함께 한국어 에러', () => {
  withEnv({ BYEORIM_AI_ADAPTER: 'gemini' }, () => {
    assert.throws(
      () => selectAdapter(),
      (err) => {
