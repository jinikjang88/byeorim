...
- 상태: 채택 (extractIntent 시그니처가 ADR 0026으로 갱신, generateCatalog 메서드가 ADR 0027로 추가됨, generateRealityCheck가 ADR 0027 본문 결정 4로 추가됨, recommendBlocks가 ADR 0029로 추가됨, recommendArchitecture가 ADR 0032로 추가됨)
- 날짜: 2026-04-28
- 결정자: DevSmith

## 맥락

CLAUDE.md 섹션 2의 절대 금지선 6번이 "특정 AI 벤더에 종속시키지 않는다"고 박았다. .byeorim/ 디렉토리는 Claude로도, GPT로도, Gemini로도, 미래의 어떤 모델로도 읽을 수 있어야 한다. 모델 어댑터는 packages/ai/ 안에서만 산다.
...
- 단점: bin/byeorim.js 같은 엔트리에서 어댑터 선택 로직이 필요
...
prospect의 본체 함수(runProspect)는 어댑터를 인자로 받는다. bin/byeorim.js가 어댑터를 만들어 주입한다. 단위 테스트는 mock 어댑터를 직접 주입한다.
